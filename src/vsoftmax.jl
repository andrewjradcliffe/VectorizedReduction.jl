...
#
############################################################################################
function vvsoftmax(A::AbstractArray{T, N}, dims::NTuple{M, Int}) where {T, N, M}
    Dᴬ = size(A)
    B = vvlogsumexp(A, dims)
    C = similar(A, Base.promote_op(exp, T))
    _vvsoftmax!(C, A, B, dims)
    return C
end

# reduction over all dims
function vvsoftmax(A::AbstractArray{T, N}, ::Colon) where {T, N}
    b = vvlogsumexp(A)
    C = similar(A, Base.promote_op(exp, T))
    ...
    for m ∈ 1:M
        ...
        if param <: StaticInt
            ...
            for n ∈ 1:N
                ...
                qnew = Expr(ifsym, :(dimm == $n), :(return _vvsoftmax!(C, A, B, $tc)))
                for r ∈ m+1:M
                    push!(tc.args, :(dims[$r]))
                end
                push!(qold.args, qnew)
            ...
            push!(qold.args, Expr(:block, :(return _vvsoftmax!(C, A, B, $tc))))
            return q
        end
    end
...
@generated function _vvsoftmax!(C::AbstractArray{Tₒ, N}, A::AbstractArray{T, N}, B::AbstractArray{Tₘ, N}, dims::D) where {Tₒ, T, Tₘ, N, M, D<:Tuple{Vararg{Integer, M}}}
    branches_softmax_quote(N, M, D)
end
@generated function _vvsoftmax!(C::AbstractArray{Tₒ, N}, A::AbstractArray{T, N}, B::AbstractArray{Tₘ, N}, dims::Tuple{}) where {Tₒ, T, Tₘ, N}
    :(copyto!(C, A); return C)
end
