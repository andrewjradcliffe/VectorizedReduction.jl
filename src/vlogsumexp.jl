    ...
    for m ∈ 1:M
        ...
        if param <: StaticInt
            ...
            for n ∈ 1:N
                ...
                qnew = Expr(ifsym, :(dimm == $n), :(return _vvlogsumexp!(C, A, B, $tc)))
                for r ∈ m+1:M
                    push!(tc.args, :(dims[$r]))
                end
                push!(qold.args, qnew)
            ...
            push!(qold.args, Expr(:block, :(return _vvlogsumexp!(C, A, B, $tc))))
            return q
        end
    end
...
@generated function _vvlogsumexp!(C::AbstractArray{Tₒ, N}, A::AbstractArray{T, N}, B::AbstractArray{Tₘ, N}, dims::D) where {Tₒ, T, Tₘ, N, M, D<:Tuple{Vararg{Integer, M}}}
    branches_logsumexp_quote(N, M, D)
end
@generated function _vvlogsumexp!(C::AbstractArray{Tₒ, N}, A::AbstractArray{T, N}, B::AbstractArray{Tₘ, N}, dims::Tuple{}) where {Tₒ, T, Tₘ, N}
    :(copyto!(C, A); return C)
end
    ...
    for m ∈ 1:M
        ...
        if param <: StaticInt
            ...
            for n ∈ 1:N
                ...
                tc = copy(t)
                push!(tc.args, :(StaticInt{$n}()))
                qnew = Expr(ifsym, :(dimm == $n), :(return _vvlogsumexp2!(B, A, $tc)))
                for r ∈ m+1:M
                    push!(tc.args, :(dims[$r]))
                end
                push!(qold.args, qnew)
            ...
            push!(qold.args, Expr(:block, :(return _vvlogsumexp2!(B, A, $tc))))
            return q
        end
    end
...
@generated function _vvlogsumexp2!(B::AbstractArray{Tₒ, N}, A::AbstractArray{T, N}, dims::D) where {Tₒ, T, N, M, D<:Tuple{Vararg{Integer, M}}}
    branches_logsumexp2_quote(N, M, D)
end
@generated function _vvlogsumexp2!(B::AbstractArray{Tₒ, N}, A::AbstractArray{T, N}, dims::Tuple{}) where {Tₒ, T, N}
    :(copyto!(B, A); return B)
end
