...
vfindmax(A, dims) = vvfindminmax(identity, >, typemin, A, dims)
vfindmin(A, dims) = vvfindminmax(identity, <, typemax, A, dims)
...
function vvfindminmax2(f::F, op::OP, init::I, A::AbstractArray{T, N}, dims::NTuple{M, Int}) where {F, OP, I, T, N, M}
    Dᴬ = size(A)
    Dᴮ′ = ntuple(d -> d ∈ dims ? 1 : Dᴬ[d], Val(N))
    B = similar(A, Base.promote_op(f, T), Dᴮ′)
    C = similar(A, Int, Dᴮ′)
    _vvfindminmax2!(f, op, init, B, C, A, dims)
    return B, CartesianIndices(A)[C]
end
function vvfindminmax3(f::F, op::OP, init::I, A::AbstractArray{T, N}, dims::NTuple{M, Int}) where {F, OP, I, T, N, M}
    Dᴬ = size(A)
    Dᴮ′ = ntuple(d -> d ∈ dims ? 1 : Dᴬ[d], Val(N))
    B = similar(A, Base.promote_op(f, T), Dᴮ′)
    C = similar(A, Int, Dᴮ′)
    _vvfindminmax3!(f, op, init, B, C, A, dims)
    return B, CartesianIndices(A)[C]
end
    ...
    for m ∈ 1:M
        ...
        if param <: StaticInt
            ...
            for n ∈ 1:N
                ...
                tc = copy(t)
                push!(tc.args, :(StaticInt{$n}()))
                qnew = Expr(ifsym, :(dimm == $n), :(return _vvfindminmax2!(f, op, init, B, C, A, $tc)))
                for r ∈ m+1:M
                    push!(tc.args, :(dims[$r]))
                end
                push!(qold.args, qnew)
            ...
            push!(qold.args, Expr(:block, :(return _vvfindminmax2!(f, op, init, B, C, A, $tc))))
            return q
        end
    end
...
@generated function _vvfindminmax2!(f::F, op::OP, init::I, B::AbstractArray{Tₒ, N}, C::AbstractArray{Tₗ, N}, A::AbstractArray{T, N}, dims::D) where {F, OP, I, Tₒ, Tₗ, T, N, M, D<:Tuple{Vararg{Integer, M}}}
    branches_findminmax2_quote(OP, I, N, M, D)
end
@generated function _vvfindminmax2!(f::F, op::OP, init::I, B::AbstractArray{Tₒ, N}, C::AbstractArray{Tₗ, N}, A::AbstractArray{T, N}, dims::Tuple{}) where {F, OP, I, Tₒ, Tₗ, T, N}
    :(copyto!(B, A); copyto!(C, LinearIndices(A)); return B, C)
end


function staticdim_findminmax3_quote(I, static_dims::Vector{Int}, N::Int)
    A = Expr(:ref, :A, ntuple(d -> Symbol(:i_, d), N)...)
    Bᵥ = Expr(:call, :view, :B)
    Cᵥ = Expr(:call, :view, :C)
    Bᵥ′ = Expr(:ref, :Bᵥ)
    ...
    if !isempty(nrinds)
        ...
        # Push to inside innermost loop
        # cmpr = Expr(:(=), :newmax, Expr(:call, :(>), A, :ξ))
        cmpr = Expr(:(=), :newmax, Expr(:call, :op, Expr(:call, :f, A), :ξ))
        push!(block.args, cmpr)
        # setmax = Expr(:(=), :ξ, Expr(:call, :ifelse, :newmax, A, :ξ))
        setmax = Expr(:(=), :ξ, Expr(:call, :ifelse, :newmax,
                                     Expr(:call, :f, A), :ξ))
...
function branches_findminmax3_quote(I, N::Int, M::Int, D)
    static_dims = Int[]
    for m ∈ 1:M
        param = D.parameters[m]
        if param <: StaticInt
            ...
            for n ∈ 1:N
                ...
                tc = copy(t)
                push!(tc.args, :(StaticInt{$n}()))
                qnew = Expr(ifsym, :(dimm == $n), :(return _vvfindminmax3!(f, op, init, B, C, A, $tc)))
                for r ∈ m+1:M
                    push!(tc.args, :(dims[$r]))
                end
                push!(qold.args, qnew)
            ...
            push!(qold.args, Expr(:block, :(return _vvfindminmax3!(f, op, init, B, C, A, $tc))))
            return q
        end
    end
    return staticdim_findminmax3_quote(I, static_dims, N)
end

@generated function _vvfindminmax3!(f::F, op::OP, init::I, B::AbstractArray{Tₒ, N}, C::AbstractArray{Tₗ, N}, A::AbstractArray{T, N}, dims::D) where {F, OP, I, Tₒ, Tₗ, T, N, M, D<:Tuple{Vararg{Integer, M}}}
    branches_findminmax3_quote(I, N, M, D)
end
@generated function _vvfindminmax3!(f::F, op::OP, init::I, B::AbstractArray{Tₒ, N}, C::AbstractArray{Tₗ, N}, A::AbstractArray{T, N}, dims::Tuple{}) where {F, OP, I, Tₒ, Tₗ, T, N}
    :(copyto!(B, A); copyto!(C, LinearIndices(A)); return B, C)
end
